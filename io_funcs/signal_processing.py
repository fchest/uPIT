...
# def istft(in_stft, win_size=256, overlap=2):
    # assert  isinstance(in_stft, np.ndarray)

    # # win_size = 512
    # # overlap=2
    # win_shift = int(np.round(win_size / overlap))

    # out = np.zeros(win_shift*in_stft.shape[0] + (win_size-win_shift))

    # for n, i in enumerate(range(0, len(out) - win_size + 1, win_shift)):
        # out[i:i + win_size] += np.fft.irfft(in_stft[n,:])
